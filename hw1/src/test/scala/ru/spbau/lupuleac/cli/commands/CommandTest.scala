...
class CommandTest extends FlatSpec with Matchers {
  ...
  "Echo command" should "print it's arguments" in {
    EchoCommand(EmptyInput(), List("a", "b"))(interpreter) should be("a b")
  }

  "Echo command" should "print empty string if there is no arguments" in {
    EchoCommand(Stdin("hello"), List())(interpreter) should be("")
  }

  "Wc command" should "print number of lines, words and bytes in file" in {
    WcCommand(EmptyInput(), List("src/test/resources/a.txt"))(interpreter) should be("1 2 11 src/test/resources/a.txt\n1 2 11 total")
  }

  "Wc command" should "also print total number for several files correctly" in {
    WcCommand(EmptyInput(), List("src/test/resources/a.txt", "src/test/resources/several_lines"))(interpreter) should be("1 2 11 src/test/resources/a.txt\n5 5 21 src/test/resources/several_lines\n6 7 32 total")
  }

  "Wc command" should "print number of lines, words and bytes in file from stdin if no arguments are provided" in {
    WcCommand(Stdin("aaa a"), List())(interpreter) should be("1 2 5")
  }
  ...
  "Wc command" should "print an error if arguments are incorrect" in {
    WcCommand(EmptyInput(), List())(interpreter) should be("bash: wc: invalid arguments")
  }

  "Cat command" should "print file content" in {
    CatCommand(EmptyInput(), List("src/test/resources/a.txt"))(interpreter) should be("hello world")
  }

  "Cat command" should "print the stdin if no arguments are provided" in {
    CatCommand(Stdin("hello from stdin"), List())(interpreter) should be("hello from stdin")
  }

  "Cat command" should "print contents of all files" in {
    CatCommand(EmptyInput(), List("src/test/resources/a.txt", "src/test/resources/several_lines"))(interpreter) should be("hello world\nmy\nlines\nin\nthis\nfile")
  }

  "Cat command" should "ignore stdin if arguments are provided" in {
    CatCommand(Stdin("aa"), List("src/test/resources/several_lines"))(interpreter) should be("my\nlines\nin\nthis\nfile")
  }
  ...
  "Process command" should "find a string in file" in {
    ...
    val res = ProcessCommand(command, EmptyInput(), List())(interpreter)
  ...
  "Pwd command" should "print a current directory" in {
    PwdCommand(EmptyInput())(interpreter) should endWith("hw1")
  }
  ...
  "Cd command" should "change current directory" in {
    CdCommand(EmptyInput(), List("src"))(interpreter)
    interpreter.currentDirectory.toString should endWith("src")
  }
